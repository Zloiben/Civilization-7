...
class Map:
    ...
    def get_resurses(self, x: int, y: int) -> int:
        return self.map[y][x].get_resurses()  # Возражает тип клетки
    ...
    def get_player_pos(self):
        return self.player_pos  # Возражает позицию игрока

    def set_player_pos(self, x: int, y: int):
        self.player_pos = (x, y)  # Изменяет позицию игрока
...
#     for i in gameMap.render_map.all_sprites.sprites():
#         print(i.rect)
...
class Player(Entity, pygame.sprite.Sprite):
    ...
    def __init__(self, pos_x: int, pos_y: int):
        ...
        super(Entity, self).__init__(gameMap.render_map.gg_sprites, gameMap.render_map.all_sprites)
...
class Tile(pygame.sprite.Sprite):
    ...
    tile_images = {
        1: load_image("Textures", "Grass.png"),
        2: load_image("Textures", "Cobblestone.png"),
        0: load_image("Textures", "Water.png")
    }

    def __init__(self, tile_map, tile_type: str, tile_num: int, pos_x: int, pos_y: int):
        super().__init__()
        dic = {"T": tile_map.lays_of_sprites[0],
               "R": tile_map.lays_of_sprites[1],
               "B": tile_map.lays_of_sprites[2]}
        self.add(tile_map.all_sprites, dic[tile_type])
        self.image = Tile.tile_images[tile_num]
        self.rect = self.image.get_rect().move(
            tile_width * pos_x, tile_height * pos_y)
...
class Hero(pygame.sprite.Sprite):
    ...
    def __init__(self, tile_map, pos_x: int, pos_y: int):
        super().__init__()
        self.add(tile_map.all_sprites, tile_map.gg_sprites)
...
camera = Camera()
camera.update(gameMap.render_map.map_1[gameMap.get_player_pos()[0]][gameMap.get_player_pos()[1]])
